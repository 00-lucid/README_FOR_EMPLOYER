...
public class AlbumStoryContext(IConfiguration configuration): IAlbumStoryContext
{
    ...
    public async Task<List<AlbumStoryModel>> GetAlbumStoryByIdAsync(int albumId)
    {
        var stories = new List<AlbumStoryModel>();
        ...
        using var cmd = new SqlCommand(@"
            ...
            WHERE AlbumId = @AlbumId
              AND CreatedAt >= DATEADD(HOUR, -24, GETDATE()) -- 24시간 이내의 레코드만 가져오기
        ...
        {
            var story = new AlbumStoryModel()
            {
                AlbumStoryId = reader.GetInt32(reader.GetOrdinal("AlbumStoryId")),
    ...
    public async Task<bool> UploadStoryAsyncByAlbumId(int albumId, Stream fileStream, string fileName)
    {
        try
        {
            // 1) 로컬에 사진 파일 저장
            ...
            cmd.Parameters.AddWithValue("@MediaUrl", $"https://iv-webapp.azurewebsites.net/_content/IV.Shared/images/{uniqueFileName}");
    ...
    public async Task<bool> DeleteStoryAsyncByStoryId(int albumId, int storyId)
    {
        try
        {
            ...
            using var selectCmd = new SqlCommand(@"
                SELECT MediaUrl
                FROM AlbumStory
                WHERE AlbumStoryId = @StoryId
                  AND AlbumId = @AlbumId
            ", conn);

            selectCmd.Parameters.AddWithValue("@StoryId", storyId);
            selectCmd.Parameters.AddWithValue("@AlbumId", albumId);

            await conn.OpenAsync();
            var storyUrlObj = await selectCmd.ExecuteScalarAsync();
            if (storyUrlObj == null)
            {
                return false; // 해당 사진이 없을 경우, 삭제 대상 없음
            }

            var storyUrl = storyUrlObj.ToString() ?? string.Empty;
            conn.Close();
            ...
            if (!string.IsNullOrEmpty(storyUrl))
            {
                ...
                var segments = storyUri.Segments;
            ...
            using var deleteCmd = new SqlCommand(@"
                DELETE FROM AlbumStory
                WHERE AlbumStoryId = @StoryId
                  AND AlbumId = @AlbumId
            ", deleteConn);

            deleteCmd.Parameters.AddWithValue("@StoryId", storyId);
    ...
    public async Task<bool> AddFileInfo(int albumId, string fileUri)
    {
        try
        {
            ...
            // PhotoUrl 경로에 최종 access url을 넣어줍니다.
            // 예) https://{도메인}/_content/IV.Shared/images/<파일명>
            cmd.Parameters.AddWithValue("@MediaUrl", fileUri);

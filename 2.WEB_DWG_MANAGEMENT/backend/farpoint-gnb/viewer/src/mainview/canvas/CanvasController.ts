...
import { Point, getEntities } from './utils'
...
export abstract class CanvasController {
    ...
    //             params.position[0] = center[0]
    //             params.position[1] = center[1]
    //             params.target[0] = center[0]
    //             params.target[1] = center[1]
...
export class SelectCanvasController extends CanvasController {
    ...
    protected onClick(x: number, y: number) {
        ...
        this.onSelectEntity(handles, x / window.devicePixelRatio, y / window.devicePixelRatio, 'select')
    }

    protected onMouseOver(x: number, y: number) {
        let handles = this.getHandles(x, y)
        let targetEquipment = null
        /**
            만약 좌표의 핸들값이 지능화된 설비에 포함되어 있다면
            지능화된 설비에 포함된 다른 모든 핸들값도 가져와야 됨 
        */
        targetEquipment = this.equipmentsByHandle.get(handles[0])

        if (targetEquipment) {
            const equipmentHandles = targetEquipment[0].handles.map((equipmentHandle) => equipmentHandle.handle)

            this.onMouseOverHandler(equipmentHandles, x / window.devicePixelRatio, y / window.devicePixelRatio)
            return
        }
...
export class MarkupCanvasController extends CanvasController {
    ...
    private setUndoList: Dispatch<SetStateAction<any>>
    private setRedoList: Dispatch<SetStateAction<any>>
...
export class EraseCanvasController extends CanvasController {
    ...
    private onMarkupErase: (x: number, y: number) => void
    private setUndoList: Dispatch<SetStateAction<any>>
    private setRedoList: Dispatch<SetStateAction<any>>
    private undoList: any
    private setMarkupChanged: (value: boolean) => void
    ...
    public constructor(
        ...
        onMarkupErase: (x: number, y: number) => void,
        undoList: any,
        setUndoList: Dispatch<SetStateAction<any>>,
        setRedoList: Dispatch<SetStateAction<any>>,
        setMarkupChanged: (value: boolean) => void
...
export class EditCanvasController extends CanvasController {
    ...
    private setUndoList: Dispatch<SetStateAction<any>>
    private setRedoList: Dispatch<SetStateAction<any>>
    private setEditMarkupIdx: Dispatch<SetStateAction<any>>
    private onMarkupErased: (x: number, y: number) => void
    private setMarkupChanged: (value: boolean) => void
    ...
    public constructor(
        ...
        onMarkupErased: (x: number, y: number) => void,
        setMarkupChanged: (value: boolean) => void
    ) {
        super(viewer, canvas, lib, onViewParamsChange)
        this.markupPaths = markupPaths
        this.onMarkupEdit = onMarkupEdit
        this.onMarkupErased = onMarkupErased
        this.undoList = undoList
        this.setUndoList = setUndoList
        this.setRedoList = setRedoList
    ...
    protected onShift(str?: string | undefined): void {
        ...
        window.localStorage.setItem('key', str == 'shift' ? 'true' : 'false')
        // console.log('isShifted:', this.isShifted);
    }
    protected onDragStart(point: any) {
        console.log('isShifted2:', this.isShifted)
    ...
    protected onDrag(point: any) {
        console.log('isShfited1:', this.isShifted)
    ...
    protected onMouseOver(x: number, y: number): void {
        ...
        if (path) {
            ...
            if (
                temp.x >= xMin - resizeSize &&
                temp.x <= xMax + resizeSize &&
                temp.y >= yMin - resizeSize &&
                temp.y <= yMax + resizeSize
            ) {
                const point = []
    ...
    private resizeMarkup = (x: number, y: number): boolean => {
        ...
        let path = this.markupPaths[idx]
        let values = this.markupPaths[idx].values
        let i = this.selDot
        if (i != -1) {
            let dx = x - point[i]
            let dy = y - point[i + 1]
            console.log('dx1:', dx, ' dy1:', dy)
            if (path.type != 'text') {
                if (
                    (path.type == 'rect' || path.type == 'circle') &&
                    (i == 0 || i == 4 || i == 8 || i == 12) &&
                    this.isShifted
                ) {
                    let dx1 = x
                    ...
                    if (i == 0 || i == 2 || i == 4) {
                        ...
                        if ((path.type == 'rect' || path.type == 'circle') && this.isShifted) {
                            if (i == 2) {
                                console.log('p14:', point[14], ' p6:', point[6])

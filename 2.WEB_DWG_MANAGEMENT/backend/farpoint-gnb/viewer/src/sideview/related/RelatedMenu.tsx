...
import { RelatedFolder, RelatedSearchResult, pushCommand } from '../..'
import RelatedSearchView from './RelatedSearchView'
import { StatusContext, setBanner } from '../..'
...
export function RelatedMenu() {
    ...
    React.useEffect(() => {
        async function fetchData() {
            const values = await Repository.getRelatedRoot()
    ...
    const companyValueChange = async (value: string) => {
        ...
        if (0 < value.length) {
            const values = await Repository.getRelatedFolders(value)
    ...
    const onSearchClick = async () => {
        ...
        const results = await Repository.searchRelated(folders, docName, docNumber)
    ...
    return (
        <div className="RelatedMenu SideViewShadow" style={style(status.currentMenu)}>
            <RelatedSearchView
                hidden={'related' !== status.currentMenu}

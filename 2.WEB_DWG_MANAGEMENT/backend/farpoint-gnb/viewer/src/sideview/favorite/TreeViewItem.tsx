import { TreeViewItemLabel } from '../../common/TreeView'
...
export const makeDocumentLabel = (text: string, depth: number, onRemoveClick: () => void): TreeViewItemLabel => {
    const labelHeight = 40
...
export const makeEquipmentLabel = (text: string, depth: number, onRemoveClick: () => void): TreeViewItemLabel => {
    const labelHeight = 40

    const normal = (
        <div className="Label" style={{ height: labelHeight + 'px' }}>
            <div className="Image1" />
            {equipmentIcon('var(--Icon-Normal)', depth)}
            {getNormalText(text, depth)}
            <div
                className="CloseButton"
                onClick={(e) => {
...
function equipmentIcon(color: string, depth: number): React.SVGProps<SVGSVGElement> {
    return (
        <svg className="Image1" fill={color} style={{ left: `calc(0px + var(--TreeView-Indent-Width) * ${depth})` }}>

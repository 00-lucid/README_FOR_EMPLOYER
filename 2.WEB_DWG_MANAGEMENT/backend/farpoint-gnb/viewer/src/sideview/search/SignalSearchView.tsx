...
import { SearchSignalResult } from '../../types'
...
export default function SignalSearchView({ hidden, onCloseView, results }: Props) {
    const onItemClick = async (item: SearchSignalResult) => {
        ...
        if (ext === 'HWP' || ext === 'JPG' || ext === 'XLS' || ext === 'PDF') {
            ...
            pushCommand({ name: 'requestOk', value: { message: `지원하지 않는 형식(${ext})입니다.`, ok: () => {} } })
        }
    }
    ...
    return (
        <div className="SignalSearchView SideViewShadow" style={style}>
            ...
            <span className="SideMenuLabel">시그널검색 결과 ({getItems().length} 건)</span>
            <CloseResultView />

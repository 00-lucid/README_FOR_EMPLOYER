...
const Select = ({ id, items, placeHolder, value, onChange }: SelectProps) => {
    ...
    return (
        <div className="CommonComponent" id={id} onClick={showListView}>
            <div
                className="SearchSelectControl"
                tabIndex={0}
                onBlur={(e) => {
                    e.stopPropagation()
                    hideListView()
            ...
            >
                <div className={'Text ' + (isHolder ? 'HolderText' : 'LabelText')}>{getText(value)}</div>
                {openButton}
                <div
                    hidden={!isActive}
                    className="SearchSelectItemList"
                    id={id + 'body'}

...
window['getVisualizeLibInst'] = function getVisualizeLibInst(params) {
    function downloadWasm(url, progress) {
        ...
        function pipeProgress(response) {
            progress &&
                response
                    .clone()
                    .body.pipeTo(new WritableStream(new ProgressStreamSink(progress, Number(response.headers.get('Content-Length')))))
            return response
        }
            ...
            '%U': function (date) {
                ...
                if (compareByDay(firstSunday, endDate) < 0) {
                    var februaryFirstUntilEndMonth =
                        __arraySum(__isLeapYear(endDate.getFullYear()) ? __MONTH_DAYS_LEAP : __MONTH_DAYS_REGULAR, endDate.getMonth() - 1) -
                        31
    ...
    var _emscripten_GetProcAddress = (Module['_emscripten_GetProcAddress'] = function () {
        return (_emscripten_GetProcAddress = Module['_emscripten_GetProcAddress'] = Module['asm']['emscripten_GetProcAddress']).apply(
            null,
            arguments
        )
    })
